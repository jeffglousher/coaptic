...
Timed proof under load: `cargo run -p coaptic-plugtest --features oscore --bin dogfood -- --oscore` (CI smokes `--iterations 2`). The harness fails if protect/unprotect stays cold or a plain completion sneaks through.
