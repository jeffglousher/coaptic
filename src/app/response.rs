...
/// Cap for assembled client Block2 bodies and `/.well-known/core` scratch
/// (not a field of [`Response`]; see [`super::App::take_response`]).
...
enum Payload<'a> {
    ...
    /// (poll / `Site::dispatch` scratch). Not an owned 4KiB field.
    Borrowed(&'a [u8]),
}
