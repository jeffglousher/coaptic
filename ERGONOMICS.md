...
| **App façade** | Present | `src/app/mod.rs::App`; `App::profile` / `block_wise` / `route` / `bind`; `get` / `put` method routers; `fn(Request<'_>) -> Response`; `Response`; `App::poll`; `well_known_core` |
